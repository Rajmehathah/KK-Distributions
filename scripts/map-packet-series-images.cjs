...
function run() {
  ...
  if (fs.existsSync(productsTsPath)) {
    ...
    PRODUCTS_TO_MATCH.forEach(p => {
      if (matched[p.key]) {
        const imgPath = matched[p.key].targetPath;
        const regex = new RegExp(`(id:\\s*'bansuri-packet-${p.key}[^']*'[\\s\\S]*?image:\\s*')([^']+)(')`, 'g');
        productsTsContent = productsTsContent.replace(regex, `$1${imgPath}$3`);
      }
    });
  ...
  if (fs.existsSync(productsJsonPath)) {
    let productsJsonContent = fs.readFileSync(productsJsonPath, 'utf8');
    PRODUCTS_TO_MATCH.forEach(p => {
      if (matched[p.key]) {
        const imgPath = matched[p.key].targetPath;
        const regex = new RegExp(`("id":\\s*"bansuri-packet-${p.key}[^"]*"[\\s\\S]*?"image":\\s*")([^"]+)(")`, 'g');
        productsJsonContent = productsJsonContent.replace(regex, `$1${imgPath}$3`);
      }
    });
    fs.writeFileSync(productsJsonPath, productsJsonContent, 'utf8');
